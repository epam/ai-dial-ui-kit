#!/usr/bin/env node
/**
 * Runtime rendering smoke test for the consumer-mixed-grid fixture.
 * Proves the mixed graph actually works end-to-end at runtime - not just
 * that the static module graph has the right shape (that's
 * assert-static-graph.mjs's job) - by executing the fixture's own
 * production bundle in a jsdom document, programmatically clicking the
 * `#load-grid-trigger` button to cross the dynamic `./grid` import
 * boundary, and reading back the dynamically-rendered row's cell text.
 *
 ...
 * outside a bundler context. Instead this loads `dist/assets/index-*.js`,
 * the fixture's own Vite-built bundle: Vite/Rollup already resolved that
 * gap correctly at fixture-build time (a real consumer's bundler always
 * does), so the emitted bundle is fully self-contained and safe to execute
 * directly. The dynamically-imported Grid chunk is loaded the same way the
 * bundle itself would load it in a browser - via a real `import()` inside
 * jsdom - once the trigger button is clicked.
 *
...
const EXPECTED_CELL_TEXT = 'Smoke-Test-Cell-Mixed';
...
/* Wait for the eager render (Button + trigger button) to be in the DOM
 * before clicking - React's initial commit is also microtask-driven. */
let trigger = null;
for (let attempt = 0; attempt < 20 && !trigger; attempt += 1) {
  await new Promise((r) => setTimeout(r, 50));
  trigger = globalThis.document.getElementById('load-grid-trigger');
}
if (!trigger) {
  console.error('FAIL: #load-grid-trigger never appeared in the DOM.');
  process.exit(1);
}
...
/* Crossing the dynamic import() boundary plus AG Grid's own render pass is
 * both microtask- and requestAnimationFrame-driven - poll rather than
 * guessing a single fixed delay. */
let found = false;
...
if (found) {
  ...
  console.log(
    "This proves the dynamic import('@epam/ai-dial-ui-kit/grid') resolved at runtime, " +
      'ModuleRegistry.registerModules(...) executed, and AG Grid rendered real row data.',
  );
  process.exit(0);
}
