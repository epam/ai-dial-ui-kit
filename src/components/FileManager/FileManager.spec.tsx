...
const forbiddenSymbolsItems: DialFile[] = [
  {
    id: 'fs-root',
    name: 'Files',
    path: 'Files',
    ...
    items: [
      {
        id: 'fs-forbidden-file',
        name: 'report<script>.pdf',
        path: 'Files/report<script>.pdf',
        parentPath: 'Files',
        nodeType: DialFileNodeType.ITEM,
        resourceType: DialFileResourceType.FILE,
        extension: 'pdf',
      ...
      {
        id: 'fs-default-forbidden-file',
        name: 'report<scrip%t>.pdf',
        path: 'Files/report<scrip%t>.pdf',
        parentPath: 'Files',
        nodeType: DialFileNodeType.ITEM,
        resourceType: DialFileResourceType.FILE,
        extension: 'pdf',
      ...
      {
        id: 'fs-forbidden-folder',
        name: 'My|Folder',
        path: 'Files/My|Folder',
        parentPath: 'Files',
        nodeType: DialFileNodeType.FOLDER,
        folderId: 'fs-forbidden-folder',
        updatedAt: '2025-01-01',
        permissions: [DialFilePermission.READ, DialFilePermission.WRITE],
      },
      {
...
describe('Dial UI Kit :: FileManager', () => {
  ...
  describe('disabled-row tooltips', () => {
    ...
    describe(' :: Forbidden Symbols', () => {
      describe('Forbidden symbols tooltip on hover', () => {
        test('shows default tooltip for file with forbidden symbols on hover if no forbiddenSymbolsRegExp provided', async () => {
          renderWithinSizedShell(
            <DialFileManager items={forbiddenSymbolsItems} path="Files" />,
          );

          await hoverRowByRowText('report<scrip%t>.pdf');

          await waitFor(() => {
            expect(
              screen.getByText(
                'This file contains forbidden characters in its name. Please rename it.',
              ),
            ).toBeInTheDocument();
          });
        });

        test('shows default tooltip for file with forbidden symbols on hover', async () => {
          renderWithinSizedShell(
            <DialFileManager
              items={forbiddenSymbolsItems}
              path="Files"
              forbiddenSymbolsRegExp={/[<>|:?"*]/}
            />,
          );

          await hoverRowByRowText('report<script>.pdf');
        ...
        test('shows default tooltip for folder with forbidden symbols on hover', async () => {
          renderWithinSizedShell(
            <DialFileManager
              items={forbiddenSymbolsItems}
              path="Files"
          ...
          await hoverRowByRowText('My|Folder');

          await waitFor(() => {
            expect(
              screen.getByText(
                'This folder contains forbidden characters in its name. Please rename it.',
              ),
            ).toBeInTheDocument();
          });
        });

        test('shows custom tooltip for file with forbidden symbols when forbiddenSymbolsTooltip provided', async () => {
          const customTooltip = 'Invalid characters detected in filename';
          ...
          renderWithinSizedShell(
            <DialFileManager
              ...
              path="Files"
              forbiddenSymbolsRegExp={/[<>|:?"*]/}
              forbiddenSymbolsTooltip={customTooltip}
            />,
          );
          ...
          expect(
            screen.queryByText(/contains forbidden characters/),
          ).not.toBeInTheDocument();
        });

        test('shows custom tooltip for folder with forbidden symbols', async () => {
          const customTooltip = 'Folder name has invalid symbols';
        ...
        test('does NOT show tooltip for file without forbidden symbols', async () => {
          renderWithinSizedShell(
            <DialFileManager
              items={forbiddenSymbolsItems}
              path="Files"
          ...
          await hoverRowByRowText('clean.txt');
        ...
        test('switching from one forbidden item to another updates tooltip', async () => {
          renderWithinSizedShell(
            <DialFileManager
              items={forbiddenSymbolsItems}
              path="Files"
              forbiddenSymbolsRegExp={/[<>|:?"*]/}
              forbiddenSymbolsTooltip="Bad name"
            />,
          );
          ...
          // Same custom tooltip should still appear (it's custom for all)
          await waitFor(() => {
            expect(screen.getByText('Bad name')).toBeInTheDocument();
          });
        });
      ...
      describe('Forbidden symbols and validation together', () => {
        test('forbiddenSymbolsRegExp and renameValidationMessages can be used together', async () => {
          renderWithinSizedShell(
            <DialFileManager
              items={forbiddenSymbolsItems}
              path="Files"
              forbiddenSymbolsRegExp={/[<>|:?"*]/}
              forbiddenSymbolsTooltip="Contains forbidden characters"
              renameValidationMessages={{
                hiddenItemWarning: 'Will become hidden',
                emptyName: 'Cannot be empty',
                duplicateName: 'Already exists',
              }}
              showHiddenFiles={true}
            />,
          );

          // Forbidden symbols tooltip appears
          await hoverRowByRowText('report<script>.pdf');

          await waitFor(() => {
            expect(
              screen.getByText('Contains forbidden characters'),
            ).toBeInTheDocument();
          });

          // Hidden file still visible
          expect(await findInGridByRowText('.gitignore')).toBeInTheDocument();
        });

        test('multiple files with forbidden symbols show correct tooltips and no hidden files tooltip shown', async () => {
          renderWithinSizedShell(
            <DialFileManager
              items={forbiddenSymbolsItems}
              path="Files"
          ...
          // Hidden file
          await hoverRowByRowText('.gitignore');

          await waitFor(() => {
            expect(
              screen.queryByText('Will become hidden'),
            ).not.toBeInTheDocument();
          });
        });
      });
